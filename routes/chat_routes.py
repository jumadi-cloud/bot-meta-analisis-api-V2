...
def chat():
    ...
    performa_patterns = [
        ...
        r'\bberapa\b', r'\btotal\b', r'\bjumlah\b',
...
Pertanyaan user: {question}"""
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        error_msg = f"Maaf, terjadi kesalahan saat memproses data Anda. Error: {str(workflow_error)}"
        try:
            add_history(session_id, "LLM", error_msg)
        except Exception as e:
            print('WARNING: Gagal simpan error chat ke chat_history.db:', e)
        chat_history = []
        try:
            chat_history = get_history_db(session_id)
        except Exception as e:
